...
def process_images(**kwargs):
    ...
    kwargs['vecs'] = get_vectors(**kwargs)
    get_manifest(**kwargs)
    write_images(**kwargs)
...
def filter_images(**kwargs):
    ...
    for i in stream_images(image_paths=image_paths):
        ...
        if (h == 0) or (w == 0):
            print(
                timestamp(), 'Skipping {} because it contains 0 height or width'.format(i.path))
            continue
        ...
        if (w/h) > (kwargs['atlas_size']/kwargs['cell_size']):
            print(
                timestamp(), 'Skipping {} because its dimensions are oblong'.format(i.path))
            continue
        filtered_image_paths.append(i.path)
    # if there are no remaining images, throw an error
    if len(filtered_image_paths) == 0:
        raise Exception(
            'No images were found! Please check your input image glob.')
...
def get_metadata_list(**kwargs):
    ...
    if kwargs['metadata'].endswith('.csv'):
        with open(kwargs['metadata']) as f:
            ...
            for i in reader:
                l.append({headers[j]: i[j] if len(i) > j and i[j]
                         else '' for j, _ in enumerate(headers)})
...
def get_manifest(**kwargs):
    ...
    if 'umap' in layouts and layouts['umap']:
        get_heightmap(layouts['umap']['variants'][0]
                      ['layout'], 'umap', **kwargs)
...
def get_vectors(**kwargs):
    ...
    print(timestamp(), 'Creating vectors for {} images'.format(
        len(kwargs['image_paths'])))
    vector_dir = os.path.join(kwargs['out_dir'], 'image-vectors', 'inception')
    if not os.path.exists(vector_dir):
        os.makedirs(vector_dir)
    model = CVAE(2)  # Output is 2 dimensions
    model.load_weights(checkpoint_dir)
    print(timestamp(), 'Creating image array')
    vecs = []
    with tqdm(total=len(kwargs['image_paths'])) as progress_bar:
        for _, i in enumerate(stream_images(**kwargs)):
            vector_path = os.path.join(
                vector_dir, clean_filename(i.path) + '.npy')
            if os.path.exists(vector_path) and kwargs['use_cache']:
                vec = np.load(vector_path)
            else:
                im = preprocess_input(img_to_array(
                    i.original.resize((256, 256))))
                mean, logvar = model.encode(np.expand_dims(im, 0))
                vec = model.reparameterize(mean, logvar)
                vec = vec.numpy().squeeze()
...
def process_multi_layout_umap(v, **kwargs):
    ...
    for n_neighbors, min_dist in itertools.product(kwargs['n_neighbors'], kwargs['min_dist']):
        filename = 'umap-n_neighbors_{}-min_dist_{}'.format(
    ...
    uncomputed_params = [
        i for i in params if not os.path.exists(i['out_path'])]
    # determine the filepath where this model will be saved
    model_filename = 'umap-' + str(abs(hash(kwargs['images'])))
    model_path = get_path('models', model_filename, **
                          kwargs).replace('.json', '.gz')
...
def save_model(model, path):
    try:
        ...
        attributes_names = [attr for attr in model.__dir__(
        ) if attr not in params and attr[0] != '_']
        attributes = {key: model.__getattribute__(
...
def load_model(path):
    ...
    model.__setattr__('embeddings_', List(
        params.get('umap_attributes').get('embeddings_')))
...
def get_tsne_layout(**kwargs):
    ...
    print(timestamp(), 'Creating TSNE layout with ' +
...
def get_rasterfairy_layout(**kwargs):
    ...
    if os.path.exists(out_path) and kwargs['use_cache']:
        return out_path
    umap = np.array(
        read_json(kwargs['umap']['variants'][0]['layout'], **kwargs))
    if umap.shape[-1] != 2:
...
def get_custom_layout(**kwargs):
    ...
    for i in stream_images(**kwargs):
        ...
        if x and y:
            ...
            if found_coords:
                print(
                    timestamp(), 'Some images are missing coordinates; skipping custom layout')
    if not found_coords:
        return
...
def get_date_layout(cols=3, bin_units='years', **kwargs):
    ...
    '''
    date_vals = [kwargs['metadata'][i].get(
        'year', False) for i in range(len(kwargs['metadata']))]
    ...
    datestrings = [i.metadata.get('year', 'no_date')
                   for i in stream_images(**kwargs)]
    ...
    coords = np.zeros((len(datestrings), 2))
...
def get_categorical_layout(null_category='Other', margin=2, **kwargs):
    ...
    boxes = get_categorical_boxes([i['count']
                                  for i in keys_and_counts], margin=margin)
...
def get_geographic_layout(**kwargs):
    ...
    for idx, i in enumerate(stream_images(**kwargs)):
        ...
        lng = float(i.metadata.get('lng', 0)) / 180
        if lat or lng:
            coords = True
        l.append([lng, lat])
    if coords:
...
def parse():
    ...
    parser.add_argument('--images', '-i', type=str,
                        default=config['images'], help='path to a glob of images to process', required=False)
    parser.add_argument('--metadata', '-m', type=str,
                        default=config['metadata'], help='path to a csv or glob of JSON files with image metadata (see readme for format)', required=False)
    parser.add_argument('--max_images', type=int, default=config['max_images'],
                        help='maximum number of images to process from the input glob', required=False)
    ...
    parser.add_argument('--max_clusters', type=int,
                        default=config['max_clusters'], help='the maximum number of clusters to return', required=False)
    ...
    parser.add_argument('--n_neighbors', nargs='+', type=int,
                        default=config['n_neighbors'], help='the n_neighbors arguments for UMAP')
    parser.add_argument('--min_dist', nargs='+', type=float,
                        default=config['min_dist'], help='the min_dist arguments for UMAP')
    parser.add_argument('--n_components', type=int,
                        default=config['n_components'], help='the n_components argument for UMAP')
    parser.add_argument(
    ...
    parser.add_argument('--min_size', type=float,
                        default=config['min_size'], help='min size of cropped images')
    ...
    parser.add_argument('--n_clusters', type=int,
                        default=config['n_clusters'], help='number of clusters to use when clustering with kmeans')
    parser.add_argument('--geojson', type=str,
                        default=config['geojson'], help='path to a GeoJSON file with shapes to be rendered on a map')
